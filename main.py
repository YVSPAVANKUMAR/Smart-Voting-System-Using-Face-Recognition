...
def env_path(name: str, default: str, render_default: str | None = None) -> str:
    value = os.environ.get(name)
    if value:
        return value
    if render_default and env_flag("RENDER", False):
...
DATASET_FOLDER = env_path("DATASET_FOLDER", 'dataset', "/tmp/smart-voting-system/dataset")
OTP_FILE = env_path("OTP_FILE", "otp.txt", "/tmp/smart-voting-system/otp.txt")
...
ensure_parent_directory(OTP_FILE)
