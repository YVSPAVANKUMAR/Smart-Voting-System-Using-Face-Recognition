...
- `MONGODB_SERVER_SELECTION_TIMEOUT_MS` - fail-fast timeout for MongoDB startup checks
...
- `DATASET_FOLDER` - temporary face dataset directory
- `OTP_FILE` - temporary OTP storage file
...
- `/tmp`-backed storage for uploads, datasets, and OTP files
...
- `UPLOAD_FOLDER=/tmp/smart-voting-system/uploads`
- `DATASET_FOLDER=/tmp/smart-voting-system/dataset`
- `OTP_FILE=/tmp/smart-voting-system/otp.txt`
...
- This app stores the reusable face samples in MongoDB, so the `/tmp` files on Render are only short-lived working files.
